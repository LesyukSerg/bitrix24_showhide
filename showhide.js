...
(function() {
    ...
    div[0].innerHTML = div[0].innerHTML + ' | <a href="#" class="webform-field-action-link hide_completed non" onclick="var itm = document.getElementsByClassName(\'task-detail-checklist-item-complete\');if(itm[0].style.display != \'none\'){document.cookie = \'hide_completed=1\';this.innerHTML=\'Показать выполненные\';for(var i in itm){itm[i].style.display=\'none\';}}else{document.cookie = \'hide_completed=0\';this.innerHTML=\'Скрыть выполенные\';for(var i in itm){itm[i].style.display=\'block\';}}">Скрыть выполненные</a>';
